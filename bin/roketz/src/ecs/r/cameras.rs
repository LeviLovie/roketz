...
pub struct Cameras(pub Vec<RoketzCamera>);

impl Cameras {
    pub fn push(&mut self, camera: RoketzCamera) {
        self.0.push(camera);
    }
}
...
pub fn init_cameras(world: &mut World) {
    ...
    let cameras = camera_types
        .iter()
        .map(|camera_type| RoketzCamera::new(camera_type.clone()))
        .collect::<Vec<_>>();
...
pub fn update_cameras(targets: Query<(&Transform, &CameraTarget)>, mut cameras: ResMut<Cameras>) {
    for (transform, target) in targets.iter() {
        let camera = &mut cameras.0.iter_mut().find(|camera| camera.ty == target.0);
